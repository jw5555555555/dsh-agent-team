export const zh = {
  ...
  retry: '重试',
  agentName: '名称',
  agentDescription: '角色定位与职责说明',
  agentRoleDescriptionHint: '此说明将作为 @{handle} 的团队身份在每次运行时自动注入给模型。深度规则与长期记忆保存在私有 memory.md 中。',
  viewMemory: '查看与编辑记忆',
  editMemory: '编辑记忆',
  saveMemory: '保存记忆',
  savingMemory: '正在保存…',
  saveMemorySuccess: '私有记忆已保存',
  saveMemoryFailed: '保存记忆失败：{message}',
  agentMemoryTitle: '@{name} 的私有记忆 (memory.md)',
  agentMemoryEmpty: '暂无私有记忆内容 (memory.md 尚未创建或为空)。Agent 在运行中沉淀的经验，或你在对话中交代其记住的规范，会自动写入此处并在每轮会话中自动加载。',
  memoryPathLabel: '物理路径：',
  memoryStats: '{bytes} 字节 · {notes} 条备忘 · {skills} 个专属技能',
  memoryBudget: '{bytes} / 8,192 字节',
  memoryOverBudget: '内容已超过 8 KiB 上限（当前 {bytes} 字节），超出部分在运行时将被截断并产生维护警告',
  loadingMemory: '正在读取私有记忆…',
  loadMemoryFailed: '读取私有记忆失败：{message}',
  cancelEdit: '取消编辑',
  agentScope: '作用域',
  workspaceAgent: '工作区 Agent',
  workspaceAgentDesc: '仅在当前工作区可用',
  globalAgent: '全局 Agent',
  globalAgentDesc: '跨所有工作区共享，保持统一记忆与设定',
  globalBadge: '全局',
  globalAgentsSection: '全局 Agents',
  scopeDemoteDisabledTooltip: '已加入外部工作区频道，不可降级为工作区作用域',
  scopeCannotDemoteNotice: '该 Agent 已加入其他工作区的频道，无法更改为工作区作用域。请先从其他工作区频道中移除。',
  ...
  optionalSuffix: '（可选）',
  agentDescriptionPlaceholder: '例如：首席任务分派秘书，负责日常琐事分拣、向Worker派发任务及向用户汇报汇总',
...
export const en = {
  ...
  retry: 'Retry',
  agentName: 'Name',
  agentDescription: 'Role & Responsibilities',
  agentRoleDescriptionHint: 'This description is automatically injected as @{handle}\'s team identity on every turn. In-depth SOPs and durable memory are stored in private memory.md.',
  viewMemory: 'View & Edit Memory',
  editMemory: 'Edit Memory',
  saveMemory: 'Save Memory',
  savingMemory: 'Saving…',
  saveMemorySuccess: 'Private memory saved',
  saveMemoryFailed: 'Failed to save memory: {message}',
  agentMemoryTitle: '@{name}\'s Private Memory (memory.md)',
  agentMemoryEmpty: 'No private memory recorded yet (memory.md is absent or empty). Knowledge saved by the agent or guidelines given during conversations persist here and load automatically on every turn.',
  memoryPathLabel: 'Storage path: ',
  memoryStats: '{bytes} bytes · {notes} note(s) · {skills} private skill(s)',
  memoryBudget: '{bytes} / 8,192 bytes',
  memoryOverBudget: 'Content exceeds 8 KiB budget ({bytes} bytes); excess will trigger maintenance warning and not be injected',
  loadingMemory: 'Loading private memory…',
  loadMemoryFailed: 'Failed to load private memory: {message}',
  cancelEdit: 'Cancel edit',
  agentScope: 'Scope',
  workspaceAgent: 'Workspace Agent',
  workspaceAgentDesc: 'Available only in this workspace',
  globalAgent: 'Global Agent',
  globalAgentDesc: 'Shared across all workspaces with unified memory and persona',
  globalBadge: 'Global',
  globalAgentsSection: 'Global Agents',
  scopeDemoteDisabledTooltip: 'Cannot demote to workspace scope while enrolled in channels outside home workspace',
  scopeCannotDemoteNotice: 'This agent has joined channels in other workspaces and cannot be changed to workspace scope. Remove it from other workspace channels first.',
  ...
  agentDescriptionPlaceholder: 'e.g. Chief Dispatcher, responsible for sorting daily tasks, dispatching to workers, and summarizing results',
