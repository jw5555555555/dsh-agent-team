...
function registerModeShadow<T extends object>(
  ...
  const sharedRemotes = {
    ...
    getMemberMemory: (request: AgentTeamGetMemberMemoryRequest) => ctx.remote.agentTeam.getMemberMemory(request),
    updateMemberMemory: (request: AgentTeamUpdateMemberMemoryRequest) => ctx.remote.agentTeam.updateMemberMemory(request),
